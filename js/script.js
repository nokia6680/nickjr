...
$('.video-select').each(function() {
  ...
  $this.wrap('<div class="video-select"></div>');
  // Style box
  $('<div>', {
    class: 'video-select__gap',
  }).insertAfter($this);

  var selectGap = $this.next('.video-select__gap'),
    caret = selectGap.find('.caret');
  // Add ul list
  $('<ul>', {
    class: 'video-select__list'
  }).insertAfter(selectGap);

  var selectList = selectGap.next('.video-select__list');
  // Add li - option items
  for (var i = 0; i < selectOptionLength; i++) {
    $('<li>', {
        class: 'video-select__item',
        html: $('<span>', {
